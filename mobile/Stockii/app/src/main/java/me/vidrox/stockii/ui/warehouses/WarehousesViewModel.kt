...
class WarehousesViewModel(application: Application) : AndroidViewModel(application) {
    var requestListener: RequestListener<List<Warehouse>>? = null
    var warehousesLiveData: MutableLiveData<List<Warehouse>> = MutableLiveData()
        ...
        Coroutines.mainThread {
            try {
                ...
                if (response.results != null && !response.results.isNullOrEmpty()) {
                    requestListener?.onSuccess(response.results)
                    return@mainThread
                }
